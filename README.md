...
#### Коммит без запуска сборки в Travis
Для того, чтобы сборка не запускалась необходимо указывать специальный ключ [ci skip] в сообщение коммита
Пример: git commit -m 'documentation update [ci skip]'   
...
Ниже представленная таблица отображает различия каждой виртаульной среды.
    1. https://docs.travis-ci.com/user/reference/overview/

 
#### Configuring Build Notifications
##### Настройки по умолчанию
По умолчанию уведомления электронной почты отправляются коммиттеру, при условии, когда:
     Есть права на push или admin права для публичного репозитория;
     Есть права на push, pull или admin права для частных репозиториев;

Электронные письма отправляются, когда:
     Сборка сломалась или все остается сломаной;
     Ранее сломанная сборка была стала рабочей;
